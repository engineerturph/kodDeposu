...
int main(){
	...
	char oyuncuIsmi[3],reelOyuncuIsmi[3],saveDosyasi[1000],saveler[1000][7],puanYazi[2],zorlukYazi[2];
	int b;
	...
	char yuksekSkorlar[100] = "Skorlar:\n\n";
	char bitir[1] = "\0";
	//Main menu :D
	...
	for(i=0;i<3;i++){
		reelOyuncuIsmi[i]=oyuncuIsmi[i];
		printf("%c",oyuncuIsmi[i]);
		Sleep(30);
	}
	Sleep(1000);
	printf("\n");
...
//		displayMatrix(tiles,arrayLength);
		...
		for(i=0;i<hafiza-1;i=i+2){
			if(sayilar[i]==sayilar[i+1]&&sayilar[i]!=0){
				sayilar[i]=0;
				sayilar[i+1]=0;
			}
		}
		...
		for(i=0;i<hafiza;i++){
			for(j=i+1;j<hafiza;j++){
				if(satir[i]==satir[j]&&sutun[i]==sutun[j]&&sayilar[i]!=0&&sayilar[j]!=0){
					sayilar[i]=0;
				}
			}
		}
...
if(game==0){
	...
	ilkCumleyeIkincisiniEkleme(saveDosyasi,reelOyuncuIsmi);
	if(puan1<10){
		puanYazi[0]='0';
		puanYazi[1]=puan1+48;
	}
